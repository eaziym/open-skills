...
const getPotentialMatches = async (req, res) => {
  try {
    ...
    // After .populate(), add scoring:
    potentialMatchesBySkills.forEach((person) => {
      const score =
        person.skills.filter((skill) =>
          currentUserInterests.includes(skill._id)
        ).length *
          2 +
        person.interests.filter((interest) =>
          currentUser.skills.includes(interest._id)
        ).length;
    ...
    // Then sort by score
    potentialMatchesBySkills.sort((a, b) => b.matchScore - a.matchScore);
  ...
  try {
    ...
    if (isAccepted) {
      if (currentUser.matchRequests.includes(swipedUser._id)) {
        ...
        currentUser.matchRequests = currentUser.matchRequests.filter(
          (id) => id !== swipedUser._id
        );
        ...
        return res.status(200).json({
          message: `🎉 It's a match with @ ${swipedUser.username} !!`,
        });
      }
      ...
      return res.status(200).json({
        message: `➕ Match request sent to @ ${swipedUser.username} !`,
      });
    }
    // LEFT swipe
    else {
